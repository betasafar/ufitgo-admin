...
import { CheckCircle2, Eye, FileText, Plus, Send, X } from "lucide-react"
...
export default function OperatorPoliciesPage() {
  ...
  const [previewPolicy, setPreviewPolicy] = useState<Pick<Policy, "title" | "version" | "content" | "ownerType" | "operatorId"> | null>(null)
    ...
    <main className="space-y-6 p-5 sm:p-8">
      ...
      <section className="rounded-xl border border-[#dbe2de] bg-white p-6 shadow-sm">
        ...
        <form onSubmit={createPolicy} className="mt-5 grid gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3"><button type="button" onClick={() => setPreviewPolicy({ title: form.title || "Untitled policy", version: 0, content: form.content, ownerType: form.operatorId ? "operator" : "platform", operatorId: form.operatorId ? Number(form.operatorId) : null })} className="inline-flex h-11 items-center gap-2 rounded-lg border border-[#0d7d5f] px-4 text-sm font-bold text-[#0d7d5f] hover:bg-[#eaf9f3]"><Eye className="size-4" />Preview</button><button disabled={saving} className="inline-flex h-11 items-center gap-2 rounded-lg bg-[#0d7d5f] px-4 text-sm font-bold text-white hover:bg-[#0b6b51] disabled:opacity-50"><FileText className="size-4" />{saving ? "Saving..." : "Save draft"}</button>{message && <p className="text-sm text-[#52605a]">{message}</p>}</div>
        </form>
      </section>

      <section className="rounded-xl border border-[#dbe2de] bg-white p-6 shadow-sm"><h2 className="font-brand text-lg font-bold text-[#17201c]">Policy versions</h2><div className="mt-5 space-y-3">{loading ? <p className="text-sm text-[#68716d]">Loading policies...</p> : policies.map((policy) => <article key={policy.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#dbe2de] bg-[#f8faf9] p-4"><div><p className="font-bold text-[#17201c]">{policy.title} <span className="text-sm text-[#68716d]">v{policy.version}</span></p><p className="mt-1 text-xs text-[#68716d]">{policy.ownerType === "platform" ? "UfitGo template" : `Operator ${policy.operatorId}`} · {policy.status}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => setPreviewPolicy(policy)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[#d3dad7] px-3 text-sm font-bold text-[#52605a] hover:bg-white"><Eye className="size-4" />Preview</button>{policy.status === "draft" ? <button type="button" onClick={() => void publishPolicy(policy.id)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[#0d7d5f] px-3 text-sm font-bold text-[#0d7d5f] hover:bg-[#eaf9f3]"><Send className="size-4" />Publish</button> : <span className="inline-flex items-center gap-1 text-sm font-bold text-[#0d7d5f]"><CheckCircle2 className="size-4" />Published</span>}</div></article>)}</div></section>

      {previewPolicy && <div role="dialog" aria-modal="true" aria-label="Policy preview" className="fixed inset-0 z-50 flex items-center justify-center bg-[#17201c]/45 p-4"><section className="flex max-h-[85vh] w-full max-w-3xl flex-col rounded-xl bg-white shadow-2xl"><header className="flex items-start justify-between gap-4 border-b border-[#dbe2de] px-6 py-5"><div><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#07845f]">{previewPolicy.ownerType === "platform" ? "UfitGo marketplace disclaimer" : `Operator ${previewPolicy.operatorId} policy`}</p><h2 className="mt-1 font-brand text-xl font-bold text-[#17201c]">{previewPolicy.title}</h2>{previewPolicy.version > 0 && <p className="mt-1 text-sm text-[#68716d]">Version {previewPolicy.version}</p>}</div><button type="button" onClick={() => setPreviewPolicy(null)} aria-label="Close preview" className="grid size-9 place-items-center rounded-lg text-[#52605a] hover:bg-[#f1f5f3]"><X className="size-5" /></button></header><div className="overflow-y-auto whitespace-pre-wrap px-6 py-5 text-sm leading-7 text-[#33413a]">{previewPolicy.content}</div></section></div>}
    </main>
  )
}
